...
num_folds = 5
img_size = 224
model_types = ['swin_base_patch4_window7_224']  # 可加入其他模型，如 'swin_base_patch4_window7_224'
base_path = os.path.dirname(os.path.abspath(__file__))
test_data_path = os.path.join(base_path, "plant-seedlings-classification", "test")
weight_dir = os.path.join(base_path, "weights4")
result_dir = os.path.join(base_path, "result4")
submission_path = os.path.join(result_dir, "final_submission.csv")
...
def predict_with_tta(model, test_loader, tta_transforms):
    ...
    with torch.no_grad():
        ...
        # 處理其他 TTA 增強
        for tta_idx, tta_transform in enumerate(tta_transforms[1:], 1):
        ...
        for model_type in model_types:
            for fold in range(num_folds):
                ...
                if os.path.exists(weight_path):
                    ...
                    model = create_model(model_type, pretrained=False, num_classes=num_classes)
                    model.load_state_dict(torch.load(weight_path), strict=False)
                    model = model.to(device)
                    fold_preds = predict_with_tta(model, test_loader, tta_transforms)
                    all_ensemble_preds += fold_preds / (num_folds * len(model_types))  # 加權平均
                    total_models += 1
                    print(f"Loaded and predicted with {model_type} fold {fold}")
                    # 清理記憶體
                    del model
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
